...
function getQueueAgeHours(submittedAt?: string | null) {
  ...
  return Math.max(
    0,
    Math.round((Date.now() - parsed.getTime()) / (60 * 60 * 1000)),
  );
}
...
function QueueCard({
  ...
  const insuranceExpiry = getExpiryTone(
    getDaysUntil(carrier.insuranceExpiryDate),
  );
  ...
  const statusTone =
    carrier.verificationStatus === "submitted" ||
      ...
      <div className="grid gap-4 lg:grid-cols-[1.5fr_1fr]">
        <div className="space-y-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <div
              className={`rounded-xl border px-3 py-2 text-sm font-medium capitalize ${statusTone}`}
            >
          ...
          {carrier.bio ? (
            <p className="text-sm text-text">{carrier.bio}</p>
          ) : null}
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {checks.map((check) => (
              ...
              >
                ...
                <p
                  className={`mt-2 text-sm ${check.ok ? "text-text" : "text-error"}`}
                >
                  {check.value}
                </p>
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            ...
            <div className="rounded-xl border border-border px-3 py-3">
              ...
              <p className="mt-2 text-sm text-text">
                {carrier.verificationNotes?.trim() ||
                  "No DB-backed note saved yet."}
              </p>
            </div>
            <div className="rounded-xl border border-border px-3 py-3">
              <p className="text-xs uppercase tracking-[0.18em] text-text-secondary">
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2">
                <Badge className={licenceExpiry.className}>
                  Licence {licenceExpiry.label}
                </Badge>
                <Badge className={insuranceExpiry.className}>
...
export function VerificationQueue({
  ...
  const [notesByCarrierId, setNotesByCarrierId] = useState<
    ...
    () =>
      [...carriers].sort((left, right) => {
        const leftNeedsReview =
          left.verificationStatus === "submitted" ||
          left.verificationStatus === "pending";
        const rightNeedsReview =
          right.verificationStatus === "submitted" ||
        ...
        return (
          new Date(
            right.verificationSubmittedAt ?? right.verifiedAt ?? 0,
          ).getTime() -
          new Date(
            left.verificationSubmittedAt ?? left.verifiedAt ?? 0,
          ).getTime()
        );
      }),
    [carriers],
  ...
  const expiredCount = sortedCarriers.filter(
    (carrier) => getDocumentHealthRank(carrier) === 0,
  ).length;
  ...
  async function submitBulk(isApproved: boolean) {
    const targets = sortedCarriers.filter((carrier) =>
    ...
    try {
      await Promise.all(
        targets.map(async (carrier) => {
          const response = await fetch(
            `/api/admin/carriers/${carrier.id}/verify`,
            {
              method: "PATCH",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
          ...
          if (!response.ok) {
            throw new Error(
              payload.error ?? `Unable to update ${carrier.businessName}.`,
            );
          }
        }),
      );
      ...
      setBulkError(
        error instanceof Error
          ? error.message
          : "Unable to process bulk verification.",
      );
    } finally {
      ...
      <Card className="p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-text-secondary">
              Select carriers to process them in sequence with the existing
              approval endpoint.
            </p>
          </div>
        ...
        {bulkError ? (
          <p className="mt-3 text-sm text-error">{bulkError}</p>
        ) : null}
      </Card>
      ...
      <div className="grid gap-4">
        {sortedCarriers.map((carrier) => (
          <QueueCard
            ...
            notes={
              notesByCarrierId[carrier.id] ?? carrier.verificationNotes ?? ""
            }
        ...
        {sortedCarriers.length === 0 ? (
          <Card className="p-4">
            <p className="subtle-text">
              No carriers are waiting in the verification queue.
            </p>
          </Card>
        ) : null}
      </div>
