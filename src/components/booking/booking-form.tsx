...
export function BookingForm({
  ...
  const [itemSizeClass, setItemSizeClass] = useState<
    "" | "S" | "M" | "L" | "XL"
  >("");
  ...
  const [submissionStage, setSubmissionStage] = useState<
    "idle" | "uploading_photo" | "creating_booking" | "creating_payment"
  >("idle");
  const [retryBookingId, setRetryBookingId] = useState<string | null>(null);
  const [successState, setSuccessState] = useState<BookingSuccessState | null>(
    null,
  );
  ...
  const blockingTrustIssues = trustIssues.filter(
    (issue) => issue.severity === "blocking",
  );
  const warningTrustIssues = trustIssues.filter(
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (parsed.itemDescription !== undefined)
        setItemDescription(parsed.itemDescription);
      if (parsed.itemCategory !== undefined)
        setItemCategory(parsed.itemCategory);
      if (parsed.itemSizeClass !== undefined)
        setItemSizeClass(parsed.itemSizeClass);
      if (parsed.itemWeightBand !== undefined)
        setItemWeightBand(parsed.itemWeightBand);
      if (parsed.itemDimensions !== undefined)
        setItemDimensions(parsed.itemDimensions);
      if (parsed.itemWeightKg !== undefined)
        setItemWeightKg(parsed.itemWeightKg);
      if (parsed.pickupAccessNotes !== undefined)
        setPickupAccessNotes(parsed.pickupAccessNotes);
      if (parsed.dropoffAccessNotes !== undefined)
        setDropoffAccessNotes(parsed.dropoffAccessNotes);
      if (parsed.pickupContactName !== undefined)
        setPickupContactName(parsed.pickupContactName);
      if (parsed.pickupContactPhone !== undefined)
        setPickupContactPhone(parsed.pickupContactPhone);
      if (parsed.dropoffContactName !== undefined)
        setDropoffContactName(parsed.dropoffContactName);
      if (parsed.dropoffContactPhone !== undefined)
        setDropoffContactPhone(parsed.dropoffContactPhone);
  ...
  async function createPaymentIntent(bookingId: string) {
    ...
    if (!response.ok) {
      const payload = await response.json();
      throw new Error(
        getPaymentErrorMessage(payload.error ?? "Payment setup failed."),
      );
    }
    ...
    try {
      ...
      setError(
        caught instanceof Error
          ? caught.message
          : "Unable to resume payment setup.",
      );
    } finally {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!pickup || !dropoff) {
      setError(
        "Choose both pickup and dropoff addresses from the address suggestions.",
      );
      return;
    }
    ...
    if (blockingTrustIssues.length > 0) {
      setError(
        blockingTrustIssues[0]?.message ??
          "Resolve the booking trust blockers first.",
      );
      return;
    }
      ...
      <div className="grid gap-4 rounded-2xl border border-success/20 bg-success/5 p-4">
        <div>
          ...
          <h2 className="mt-1 text-lg text-text">
            Your request reference is ready
          </h2>
          <p className="mt-2 text-sm text-text-secondary">
            moverrr is opening the request detail now so you can track the
            carrier response window and next steps.
          </p>
        </div>
        ...
        <p className="text-sm text-text-secondary">
          Keep this reference handy if you need support or want to double-check
          the request email.
        </p>
      </div>
    );
  ...
  return (
    <form id={id} className="grid gap-4" onSubmit={handleSubmit}>
      <fieldset
        disabled={isSubmitting || Boolean(retryBookingId)}
      ...
      >
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <label className="flex flex-col gap-2">
            ...
            <select
              ...
              onChange={(event) =>
                setItemSizeClass(
                  event.target.value as "" | "S" | "M" | "L" | "XL",
                )
              }
              required
            >
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <label className="flex flex-col gap-2">
            ...
            >
              ...
              <option value="20_to_50kg">
                Bulky one-person lift (20 to 50kg)
              </option>
              <option value="50_to_100kg">
                Heavy item or careful two-person lift (50 to 100kg)
              </option>
              <option value="over_100kg">
                Very heavy or specialist check (over 100kg)
              </option>
            </select>
          </label>
          <label className="flex flex-col gap-2">
        ...
        <div className="space-y-3 rounded-xl border border-border p-3">
          <div>
            ...
            <p className="mt-1 text-sm text-text-secondary">
              Match your item to the closest everyday size so the carrier can
              judge fit quickly.
            </p>
          </div>
          <div className="grid gap-2 sm:grid-cols-4">
            {Object.entries(ITEM_SIZE_DESCRIPTIONS).map(([value, config]) => (
              <button
                ...
                type="button"
                onClick={() =>
                  setItemSizeClass(value as "S" | "M" | "L" | "XL")
                }
                className={`rounded-xl border p-3 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/30  ${
                  itemSizeClass === value
        ...
        <div className="space-y-3 rounded-xl border border-border bg-black/[0.02] p-3 dark:bg-white/[0.04]">
          <div>
            <p className="text-sm font-medium text-text">
              Safety policy before you book
            </p>
            <p className="mt-1 text-sm text-text-secondary">
              moverrr is for ordinary spare-capacity moves, not specialist
              disposal or unsafe loads.
            </p>
          </div>
        ...
        {trustIssues.length > 0 ? (
          ...
          >
            ...
            {blockingTrustIssues.length > 0 ? (
              <p className="mt-1 text-sm text-text-secondary">
                Prohibited items stay blocked because they do not belong in
                moverrr&apos;s spare-capacity marketplace.
              </p>
            ) : null}
        ...
        <label className="flex flex-col gap-2">
          ...
          />
          <span
            className={`text-xs ${pickup ? "text-success" : "text-text-secondary"}`}
          >
            {pickup
              ? "Pickup address confirmed."
              : "Select a suggested address to confirm pickup."}
          </span>
        </label>
        <div className="grid gap-3 sm:grid-cols-2">
        ...
        <label className="flex flex-col gap-2">
          ...
          {accessWarning ? (
            ...
            <span className="text-xs text-text-secondary">
              Include stairs, lifts, loading docks, gate codes, parking
              difficulty, or long carries.
            </span>
          )}
        </label>
        ...
        <label className="flex flex-col gap-2">
          ...
          />
          <span
            className={`text-xs ${dropoff ? "text-success" : "text-text-secondary"}`}
          >
            {dropoff
              ? "Dropoff address confirmed."
              : "Select a suggested address to confirm dropoff."}
          </span>
        </label>
        <div className="grid gap-3 sm:grid-cols-2">
          <label className="grid gap-2">
        ...
        <p className="text-xs text-text-secondary">
          Contact numbers are for the booked handoff only. Keep payment and any
          extra charges inside moverrr.
        </p>
        <label className="flex flex-col gap-2">
          <div className="flex items-center justify-between gap-3">
            <span className="text-sm font-medium text-text">
              Dropoff access notes
            </span>
            <span className="text-xs text-text-secondary">
              {dropoffAccessNotes.length}/240
            </span>
          </div>
          <Textarea
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <label className="flex flex-col gap-2">
            ...
            {helperWarning ? (
              ...
              <span className="text-xs text-text-secondary">
                Use this when the item is bulky, awkward, or not safe for
                one-person handling.
              </span>
            )}
          </label>
        ...
        {stairsUnsupported ? (
          <div className="rounded-xl border border-warning/20 bg-warning/10 p-3 text-sm text-text">
            This carrier does not offer stairs support. Choose &quot;No&quot;
            for stairs or find a different trip before continuing.
          </div>
        ) : null}

        <label className="flex flex-col gap-2">
          <span className="text-sm font-medium text-text">
            Anything the carrier should know?
          </span>
          <Textarea
        ...
        <div className="rounded-xl border border-border bg-black/[0.02] p-3 text-sm text-text-secondary dark:bg-white/[0.04]">
          moverrr holds payment on-platform and only supports the listed
          add-ons. If anyone asks to switch to cash, bank transfer, or
          side-payment extras, stop and report it inside the booking.
        </div>
        <div className="rounded-xl border border-border bg-black/[0.02] p-3 text-sm text-text-secondary dark:bg-white/[0.04]">
          <p className="font-medium text-text">Bulky or risky item reminder</p>
          <div className="mt-1 grid gap-2">
            <p>
              If the item is heavy, awkward, or likely unsafe for one person,
              add helper and access details now rather than leaving it for
              pickup day.
            </p>
            {MANUAL_HANDLING_POLICY_LINES.map((line) => (
              <p key={line}>{line}</p>
