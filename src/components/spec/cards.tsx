...
const matchClassStyle: Record<OfferCard["matchClass"], string> = {
  ...
  partial: "bg-[var(--bg-elevated-2)] text-text-secondary",
};
...
export function ResultCard({ offer }: { offer: OfferCard }) {
  return (
    <Link
      ...
      className="block rounded-lg border border-border bg-surface p-4 shadow-[var(--shadow-card)] transition-colors hover:bg-[var(--bg-elevated-2)] active:bg-[var(--bg-elevated-3)]"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <p className="title">{offer.carrierName}</p>
            <span className="inline-flex h-6 items-center gap-1 rounded-[var(--radius-pill)] border border-border bg-[var(--bg-elevated-2)] px-2 text-[11px] font-semibold text-text-secondary">
              <ShieldCheck size={12} />
              Verified
            </span>
          </div>
          <p className="mt-1 text-[13px] text-text-secondary">{offer.ratingLabel}</p>
          <p className="text-[13px] text-[var(--text-tertiary)]">{offer.vehicle}</p>
        </div>
        <div className="text-right">
          <p className="tabular text-[28px] font-semibold leading-none tracking-[-0.04em] text-text">
            {offer.total}
          </p>
      ...
      <div className="mt-4 rounded-md bg-[var(--bg-elevated-2)] px-4 py-3">
        <p className="text-[13px] font-semibold text-text">Why this matches</p>
        <p className="mt-1 text-[14px] text-text-secondary">{offer.why}</p>
      ...
      <div className="mt-3 flex items-center justify-between gap-3">
        <div className="flex flex-wrap gap-2">
          ...
          {offer.tags?.[0] ? (
            <span className="inline-flex min-h-[28px] items-center rounded-[var(--radius-pill)] border border-border bg-surface px-3 text-[12px] font-medium text-text-secondary">
              {offer.tags[0]}
            </span>
          ) : null}
        </div>
...
export function CollapsibleResults({
  ...
  return (
    <details open={openByDefault} className="rounded-lg border border-border bg-surface p-3 shadow-[var(--shadow-card)]">
      <summary className="flex min-h-[48px] list-none items-center justify-between gap-2 rounded-md px-2 text-[15px] font-semibold text-text hover:bg-[var(--bg-elevated-2)] active:bg-[var(--bg-elevated-3)]">
        <span>{title}</span>
...
export function InfoCard({
  ...
  return (
    <div className="rounded-lg border border-[color:rgba(201,82,28,0.22)] bg-[var(--accent-subtle)] p-4">
...
export function EmptyStateCard({
  ...
  return (
    <div className="rounded-lg border border-border bg-surface p-5 text-center shadow-[var(--shadow-card)]">
      <p className="title">{title}</p>
      <p className="mt-2 caption">{description}</p>
      {ctaHref && ctaLabel ? (
        <Link
