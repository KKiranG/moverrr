export default function CarrierRequestDetailLoading() {
  return (
    <main id="main-content" className="screen animate-pulse">
      <div className="mb-6">
        <div className="mb-2 h-3 w-24 rounded bg-black/[0.08] dark:bg-white/[0.12]" />
        <div className="mb-2 h-7 w-40 rounded bg-black/[0.08] dark:bg-white/[0.12]" />
      </div>
      ...
      <div className="space-y-4">
        <div className="rounded-xl border border-border p-4">
          ...
          {[40, 80, 60].map((w, i) => (
            <div
              key={i}
              className="mb-2 h-4 rounded bg-black/[0.08] dark:bg-white/[0.12]"
              style={{ width: `${w}%` }}
            />
          ))}
        </div>
