...
export async function createBookingForCustomer(
  ...
  const { data: bookingId, error } = await supabase.rpc("create_booking_atomic", {
    ...
    p_customer_mover_preference: parsed.data.customerMoverPreference,
    p_stairs_level_pickup: parsed.data.stairsLevelPickup,
    p_stairs_level_dropoff: parsed.data.stairsLevelDropoff,
    p_lift_available_pickup: parsed.data.liftAvailablePickup,
    p_lift_available_dropoff: parsed.data.liftAvailableDropoff,
